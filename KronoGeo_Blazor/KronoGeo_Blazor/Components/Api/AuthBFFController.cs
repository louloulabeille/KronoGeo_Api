...
namespace KronoGeo_Blazor.Components.Api
{
    ...
    [ApiController]
    public class AuthBFFController( IServiceHttpKronoGeo httpKronoGeo
        , IMemoryCache memoryCache, ILogger<AuthBFFController> logger ) : Controller
    {
        #region private readonly properties
        private readonly IServiceHttpKronoGeo _httpKronoGeo = httpKronoGeo;
        ...
        public async Task<IActionResult> Login([FromBody] RegisterDTO login)
        {
            try
            {
                if ( _httpKronoGeo is not null )
                {
                    ...
                    if (result.IsSuccess)
                    {
                        if (!string.IsNullOrEmpty(result.Register?.Token))
                        {
                            ...
                            await HttpContext.SignInAsync("BffCookie", claimsPrincipal);
                ...
                return Ok( new ResponseApiAuthenticate() { 
